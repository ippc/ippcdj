...
class BaseImporterCommand(BaseCommand):
    """
    Base importer command for callsging platform specific management
    commands to subclass when importing calls posts into Mezzanine.
    The ``handle_import`` method should be overridden to provide the
    import mechanism specific to the callsging platform being dealt with.
    """

    option_list = BaseCommand.option_list + (
        make_option("-m", "--mezzanine-user", dest="mezzanine_user",
            help="Mezzanine username to assign the imported calls posts to."),
    ...
    def handle(self, *args, **options):
        ...
        for post_data in self.posts:
            ...
            post_data = self.trunc(CallsPost, prompt, **post_data)
            initial = {
                "title": post_data.pop("title"),
                "user": mezzanine_user,
            }
            post, created = CallsPost.objects.get_or_create(**initial)
            ...
            for name in categories:
                cat = self.trunc(CallsCategory, prompt, title=name)
                if not cat["title"]:
                    continue
                cat, created = CallsCategory.objects.get_or_create(**cat)
                if created and verbosity >= 1:
    ...
    def add_meta(self, obj, tags, prompt, verbosity, old_url=None):
        """
        Adds tags and a redirect for the given obj, which is a calls
        post or a page.
        """
        for tag in tags:

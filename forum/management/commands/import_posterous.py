...
from ippc.forum.management.base import BaseImporterCommand


class Command(BaseImporterCommand):
    """
    Import Tumblr forum posts into the forum app.
    """
    ...
    option_list = BaseImporterCommand.option_list + (
        ...
        make_option("-d", "--posterous-host", dest="hostname",
            help="Posterous Forum Hostname (no http.. eg. 'foo.com')"
        ),
    )
    help = "Import Posterous forum posts into the forum app."
    ...
    def handle_import(self, options):
        ...
        if not hostname and not site:
            if len(sites) == 1:
                ...
                raise CommandError(
                    "Please pass your forum hostname if you have more than"
                    " one forum on your posterous account."
                )
        page = 1
        while True:

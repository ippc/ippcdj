...
from ippc.forum.models import ForumPost, ForumCategory
...
class BaseImporterCommand(BaseCommand):
    """
    Base importer command for forum platform specific management
    commands to subclass when importing forum posts into Mezzanine.
    The ``handle_import`` method should be overridden to provide the
    import mechanism specific to the forum platform being dealt with.
    """

    option_list = BaseCommand.option_list + (
        make_option("-m", "--mezzanine-user", dest="mezzanine_user",
            help="Mezzanine username to assign the imported forum posts to."),

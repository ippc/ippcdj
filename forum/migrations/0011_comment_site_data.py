...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        OrmCategory = orm['forum.forumcategory']
        forumcategories = OrmCategory._base_manager.all()

        # This block wll try to determine what additional many-to-many fields
        # may be pointing to the ForumCategory, unknown to mezzanine, by looking
        # at the live orm
        try:
            from forum.models import ForumCategory
            additional_fields = []
            for attr_name in dir(ForumCategory):
                attr = getattr(ForumCategory, attr_name)
                if isinstance(attr, ManyRelatedObjectsDescriptor) and \
                   attr.related.parent_model is ForumCategory and \
                   attr_name != "forumposts":
        ...
        for category in forumcategories:
            ...
            # to the ForumCategory via a many-to-many relation outside of Mezzanine. It 
            ...
            # `forumcategories` above will be empty and nothing in this for loop will execute.
            #
            ...
            try:
                from forum.models import ForumCategory
                live_category = ForumCategory._base_manager.get(pk=category.pk)
                    ...
                    try:
                        ...
                        for related in related_set.all():
                            ...
                            if not dict_category:
                                ...
                                dict_category.save()
                                site_dict[site.pk] = dict_category
                            new_category = ForumCategory._base_manager.get(pk=dict_category.pk)

...
class BaseImporterCommand(BaseCommand):
    """
    Base importer command for iyphging platform specific management
    commands to subclass when importing iyph posts into Mezzanine.
    The ``handle_import`` method should be overridden to provide the
    import mechanism specific to the iyphging platform being dealt with.
    """

    option_list = BaseCommand.option_list + (
        make_option("-m", "--mezzanine-user", dest="mezzanine_user",
            help="Mezzanine username to assign the imported iyph posts to."),
    ...
    def handle(self, *args, **options):
        ...
        for post_data in self.posts:
            ...
            post_data = self.trunc(IyphPost, prompt, **post_data)
            initial = {
                "title": post_data.pop("title"),
                "user": mezzanine_user,
            }
            post, created = IyphPost.objects.get_or_create(**initial)
            ...
            for name in categories:
                cat = self.trunc(IyphCategory, prompt, title=name)
                if not cat["title"]:
                    continue
                cat, created = IyphCategory.objects.get_or_create(**cat)
                if created and verbosity >= 1:
    ...
    def add_meta(self, obj, tags, prompt, verbosity, old_url=None):
        """
        Adds tags and a redirect for the given obj, which is a iyph
        post or a page.
        """
        for tag in tags:

...
MAX_POSTS_PER_IYPH = 20  # Max number of posts Tumblr API will return per iyph.
MAX_RETRIES_PER_IYPH = 3  # Max times to retry API iyph after failing.
...
class Command(BaseImporterCommand):
    ...
    def handle_import(self, options):
        ...
        while True:
            retries = MAX_RETRIES_PER_IYPH
            try:
                iyph_url = "%s?start=%s" % (json_url, start_index)
                if verbosity >= 2:
                    print "Calling %s" % iyph_url
                response = urlopen(iyph_url)
            ...
            posts = json["posts"]
            start_index += MAX_POSTS_PER_IYPH
            ...
            if len(posts) < MAX_POSTS_PER_IYPH:
                break
